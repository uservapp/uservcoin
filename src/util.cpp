...
const char * const BITCOIN_CONF_FILENAME = "uservcoin.conf";
const char * const BITCOIN_PID_FILENAME = "uservcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "uservcoin" is a composite category enabling all UservCoin-related debug output
            if(ptrCategory->count(string("uservcoin"))) {
...
#else
    const char* pszModule = "uservcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UservCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UservCoinCore
    // Mac: ~/Library/Application Support/UservCoinCore
    // Unix: ~/.uservcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UservCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UservCoinCore";
#else
    // Unix
    return pathRet / ".uservcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty uservcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override uservcoin.conf
