...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UserVMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("UserVMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("UserVMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("userv-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running UserVMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
