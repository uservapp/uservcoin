...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (userv.conf).");
            QMessageBox::warning(this, "UserV Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
