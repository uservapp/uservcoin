...
{
    // return if URI is not valid or is no userv: URI
...
{
    // Convert userv:// to userv:
    //
    //    Cannot handle this later, because userv:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open userv.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "UserV.lnk";
}
...
{
    return GetAutostartDir() / "userv.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a userv.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=UserV\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the userv app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add userv app to startup item list
