...
#if defined(NDEBUG)
#error "UserV cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uuserv) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 20 * COIN;
	} else if (nHeight < 189500 && nHeight >= 180000) {
        nSubsidy = 15 * COIN;
	} else {
        nSubsidy = 0.01 * COIN;
    }
...
int ActiveProtocol()
{
    ...
    // SPORK_15 is used for 70915. Nodes < 70915 don't see it and still get their protocol version via SPORK_14 and their
