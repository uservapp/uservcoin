...
*Setup instructions for a gitian build of UservCoin using a Debian VM or physical system.*
...
- [Building UservCoin](#building-uservcoin)
...
Clone the git repositories for uservcoin and gitian and then checkout the uservcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the uservcoin repository
...
To build UservCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the uservcoin repository.
...
For example:
```bash
URL=https://github.com/konez2k/uservcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/uservcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/uservcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/uservcoin-build.assert
```
...
[uservcoin/gitian.sigs](https://github.com/uservapp/gitian.sigs/) repository, or if that's not possible to create a pull
request.
