...
*Setup instructions for a Gitian build of UservCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the UservCoin
...
- [Building UservCoin Core](#building-uservcoin-core)
...
Clone the git repositories for UservCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/uservapp/uservcoin
```
...
in the UservCoin Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building UservCoin Core
----------------

To build UservCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the UservCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/uservcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/uservapp/uservcoin
...
For example:
```bash
URL=https://github.com/crowning-/uservcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit uservcoin=${COMMIT} --url uservcoin=${URL} ../uservcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit uservcoin=${COMMIT} --url uservcoin=${URL} ../uservcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit uservcoin=${COMMIT} --url uservcoin=${URL} ../uservcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the uservcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../uservcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/uservcoin.git
SIGPATH=/some/root/path/uservcoin-detached-sigs.git

./bin/gbuild --url uservcoin=${BTCPATH},signature=${SIGPATH} ../uservcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/uservcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/uservcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/uservcoin-osx-build.assert
```
...
[uservcoin/gitian.sigs](https://github.com/uservcoincoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
