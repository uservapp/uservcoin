...
*Setup instructions for a gitian build of UserV using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the UserV
...
Clone the git repositories for userv and gitian and then checkout the userv version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the userv repository
...
To build UserV (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the userv repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/userv/.git/
...
For example:
```bash
URL=https://github.com/konez2k/userv.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/userv-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/userv-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/userv-build.assert
```
...
[userv/gitian.sigs](https://github.com/uservapp/gitian.sigs/) repository, or if that's not possible to create a pull
request.
